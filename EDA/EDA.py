...
def create_output_structure():
    ...
    # Use EDA/results directory instead of timestamped directory
    base_dir = "EDA/results"
...
class IncrementalUpdateManager:
    """Manages incremental updates and change detection for EDA analysis"""
    
    def __init__(self, base_dir="EDA"):
        self.base_dir = base_dir
        self.state_file = "analysis_state.json"
        self.dataset_hash_file = "dataset_hash.txt"
        self.analysis_components = [
            ...
            'predictive_analysis',
            'genre_artist_analysis',
            'explicitness_analysis',
            'comprehensive_dashboard'
        ]
    
    def get_dataset_hash(self, dataset_path):
        """Calculate hash of dataset files to detect changes"""
        try:
            hasher = hashlib.md5()
            ...
            print(f"⚠️  Error calculating dataset hash: {e}")
            return None
    
    def load_analysis_state(self):
        """Load the current analysis state"""
        state_path = os.path.join(self.base_dir, self.state_file)
        if os.path.exists(state_path):
            try:
                with open(state_path, 'r') as f:
                    return json.load(f)
            except Exception as e:
                print(f"⚠️  Error loading analysis state: {e}")
        return {
            'last_update': None,
            'dataset_hash': None,
            'completed_analyses': [],
    ...
    def save_analysis_state(self, state):
        ...
        try:
            ...
            print(f"⚠️  Error saving analysis state: {e}")
    
    def check_dataset_changes(self, dataset_path):
        """Check if dataset has changed since last run"""
        current_hash = self.get_dataset_hash(dataset_path)
        if not current_hash:
            return True, "Could not calculate dataset hash"
        
        state = self.load_analysis_state()
        last_hash = state.get('dataset_hash')
        
        if last_hash is None:
            return True, "First run - no previous hash found"
        
        if current_hash != last_hash:
            return True, f"Dataset changed (hash: {current_hash[:8]}... vs {last_hash[:8]}...)"
        
        return False, "Dataset unchanged"
    
    def check_completed_analyses(self, output_dir):
        """Check which analyses have been completed"""
        completed = []
        missing = []
        
        # Handle case where output_dir is None (first run)
        if output_dir is None:
            return completed, self.analysis_components.copy()
        
        for component in self.analysis_components:
            # Check for summary file (primary indicator of completion)
            summary_file = os.path.join(output_dir, 'reports', f'{component}_summary.json')
            
            # For newer analyses, also check for plot data file
            plot_data_file = os.path.join(output_dir, 'data', f'{component}_plot_data.json')
            # For older analyses, check for processed_data directory
            old_plot_data_file = os.path.join(output_dir, 'processed_data', f'{component}_plot_data.json')
            
            # Check for figure file (secondary indicator)
            figure_file = os.path.join(output_dir, 'figures', f'{component}.png')
            
            # Consider completed if summary exists AND (plot data exists OR figure exists)
            summary_exists = os.path.exists(summary_file)
            plot_data_exists = os.path.exists(plot_data_file) or os.path.exists(old_plot_data_file)
            figure_exists = os.path.exists(figure_file)
            
            if summary_exists and (plot_data_exists or figure_exists):
    ...
    def get_analysis_status(self, dataset_path, output_dir):
        """Get comprehensive analysis status"""
        dataset_changed, change_reason = self.check_dataset_changes(dataset_path)
        completed, missing = self.check_completed_analyses(output_dir)
        ...
        return {
            ...
            'total_analyses': len(self.analysis_components),
            'completion_percentage': len(completed) / len(self.analysis_components) * 100
        }
    
    def update_analysis_state(self, dataset_path, output_dir, completed_component, analysis_time):
        """Update analysis state after completing a component"""
        state = self.load_analysis_state()
        ...
        if completed_component not in state['completed_analyses']:
            state['completed_analyses'].append(completed_component)
        
        # Update analysis times
        state['analysis_times'][completed_component] = {
    ...
    def print_status_report(self, status):
        ...
        print("\n" + "="*60)
        print("📊 INCREMENTAL UPDATE STATUS REPORT")
        print("="*60)
        
        print(f"🔄 Dataset Status: {'CHANGED' if status['dataset_changed'] else 'UNCHANGED'}")
        print(f"   Reason: {status['change_reason']}")
        
        print(f"\n📈 Analysis Progress: {status['completion_percentage']:.1f}% complete")
        print(f"   Completed: {len(status['completed_analyses'])}/{status['total_analyses']}")
        print(f"   Missing: {len(status['missing_analyses'])}/{status['total_analyses']}")
        
        if status['completed_analyses']:
            print(f"\n✅ Completed Analyses:")
            for analysis in status['completed_analyses']:
                print(f"   • {analysis}")
        
        if status['missing_analyses']:
            print(f"\n⏳ Missing Analyses:")
            for analysis in status['missing_analyses']:
                print(f"   • {analysis}")
...
def save_plot_data(plot_data, filename, subfolder='data'):
    """Save plot numerical data for regeneration"""
    # Ensure the directory exists
    os.makedirs(output_dirs[subfolder], exist_ok=True)
    filepath = os.path.join(output_dirs[subfolder], f"{filename}_plot_data.json")
    ...
    converted_data = convert_for_json(plot_data)
    
    with open(filepath, 'w') as f:
        json.dump(converted_data, f, indent=2, default=str)
    print(f"Saved plot data: {filepath}")

def load_plot_data(filename, subfolder='data'):
    """Load plot numerical data for regeneration"""
    filepath = os.path.join(output_dirs[subfolder], f"{filename}_plot_data.json")
    with open(filepath, 'r') as f:
        return json.load(f)

def get_graph_type_info(filename, subfolder='data'):
    """Get graph type and metadata information from saved plot data"""
    try:
        plot_data = load_plot_data(filename, subfolder)
        if 'metadata' in plot_data:
            metadata = plot_data['metadata']
            print(f"📊 Graph Type Information for '{filename}':")
            print(f"   Figure Type: {metadata.get('figure_type', 'Unknown')}")
            print(f"   Title: {metadata.get('title', 'No title')}")
            print(f"   Subplot Layout: {metadata.get('subplot_layout', 'Unknown')}")
            print(f"   Total Subplots: {metadata.get('total_subplots', 'Unknown')}")
            print(f"\n   Subplot Details:")
            
            subplot_info = metadata.get('subplot_info', {})
            for position, info in subplot_info.items():
                print(f"     [{position}]: {info.get('type', 'Unknown')} - {info.get('title', 'No title')}")
                if 'xlabel' in info:
                    print(f"       X-axis: {info['xlabel']}")
                if 'ylabel' in info:
                    print(f"       Y-axis: {info['ylabel']}")
            
            return metadata
        else:
            print(f"❌ No metadata found in plot data for '{filename}'")
            return None
    except FileNotFoundError:
        print(f"❌ Plot data file not found: {filename}")
        return None
    except Exception as e:
        print(f"❌ Error loading plot data: {e}")
        return None

def list_available_graph_types(subfolder='data'):
    """List all available graph types from saved plot data"""
    try:
        data_dir = os.path.join(output_dirs[subfolder])
        ...
        for filename in os.listdir(data_dir):
            if filename.endswith('_plot_data.json'):
                graph_name = filename.replace('_plot_data.json', '')
                try:
                    plot_data = load_plot_data(graph_name, subfolder)
                    if 'metadata' in plot_data:
                        graph_types.append({
                            'name': graph_name,
                            'type': plot_data['metadata'].get('figure_type', 'Unknown'),
                            'title': plot_data['metadata'].get('title', 'No title')
                        })
                except:
                    continue
        ...
        if graph_types:
            ...
            for graph in graph_types:
                print(f"   • {graph['name']} ({graph['type']}): {graph['title']}")
        else:
...
def regenerate_basic_data_overview(plot_data):
    """Regenerate basic data overview figure from saved data"""
    # Get metadata for validation
    metadata = plot_data.get('metadata', {})
    figure_type = metadata.get('figure_type', 'basic_data_overview')
    title = metadata.get('title', 'Basic Data Overview - Spotify Dataset')
    
    if figure_type != 'basic_data_overview':
        print(f"⚠️  Warning: Expected 'basic_data_overview' but got '{figure_type}'")
    
    fig, axes = plt.subplots(2, 3, figsize=(20, 12))
    fig.suptitle(title, fontsize=16, fontweight='bold')
    
    # 1. Missing values heatmap
    missing_data = plot_data['missing_data']
    missing_percent = plot_data['missing_percent']
    
    axes[0, 0].barh(range(len(missing_data)), missing_percent)
    axes[0, 0].set_yticks(range(len(missing_data)))
    axes[0, 0].set_yticklabels(list(missing_data.keys()), fontsize=8)
    axes[0, 0].set_xlabel('Missing Percentage (%)')
    axes[0, 0].set_title('Missing Values by Column')
    axes[0, 0].grid(True, alpha=0.3)
    
    # 2. Data types distribution
    dtype_counts = plot_data['dtype_counts']
    axes[0, 1].pie(list(dtype_counts.values()), labels=list(dtype_counts.keys()), autopct='%1.1f%%')
    axes[0, 1].set_title('Data Types Distribution')
    
    # 3. Daily counts (if available)
    if 'daily_counts' in plot_data and plot_data['daily_counts']:
        daily_counts = plot_data['daily_counts']
        dates = list(daily_counts.keys())
        counts = list(daily_counts.values())
        axes[0, 2].plot(dates, counts, marker='o')
    ...
    if 'numeric_distributions' in plot_data:
        numeric_dists = plot_data['numeric_distributions']
        for i, (col, data) in enumerate(numeric_dists.items()):
            if i < 4:  # Limit to 4 subplots
                row, col_idx = (1, i) if i < 2 else (1, i-2)
                if row < 2 and col_idx < 3:
                    axes[row, col_idx].hist(data['values'], bins=30, alpha=0.7)
                    axes[row, col_idx].set_title(f'Distribution of {col}')
                    axes[row, col_idx].set_xlabel(col)
                    axes[row, col_idx].set_ylabel('Frequency')
    
    # 5. Categorical columns analysis
    if 'categorical_data' in plot_data:
        cat_data = plot_data['categorical_data']
        if cat_data:
            top_categories = list(cat_data.items())[:10]
            categories = [item[0] for item in top_categories]
            counts = [item[1] for item in top_categories]
            axes[1, 2].barh(range(len(categories)), counts)
            axes[1, 2].set_yticks(range(len(categories)))
            axes[1, 2].set_yticklabels(categories, fontsize=8)
            axes[1, 2].set_xlabel('Count')
            axes[1, 2].set_title('Top 10 Categories')
    
    plt.tight_layout()
    return fig

def regenerate_temporal_analysis(plot_data):
    """Regenerate temporal analysis figure from saved data"""
    # Get metadata for validation
    metadata = plot_data.get('metadata', {})
    figure_type = metadata.get('figure_type', 'temporal_analysis')
    title = metadata.get('title', 'Temporal Analysis - Song Popularity Trends')
    
    if figure_type != 'temporal_analysis':
        print(f"⚠️  Warning: Expected 'temporal_analysis' but got '{figure_type}'")
    ...
    dates = list(daily_counts.keys())
    counts = list(daily_counts.values())
    axes[0, 0].plot(dates, counts, marker='o', linewidth=2)
    ...
    # 2. Weekly trends
    weekly_counts = plot_data['weekly_counts']
    weeks = list(weekly_counts.keys())
    counts = list(weekly_counts.values())
    axes[0, 1].bar(weeks, counts, alpha=0.7)
    ...
    # 3. Monthly trends
    monthly_counts = plot_data['monthly_counts']
    months = list(monthly_counts.keys())
    counts = list(monthly_counts.values())
    month_names = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 
                  'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
    axes[1, 0].bar(range(1, 13), [monthly_counts.get(str(i), 0) for i in range(1, 13)], alpha=0.7)
    ...
    # 4. Yearly trends
    yearly_counts = plot_data['yearly_counts']
    years = list(yearly_counts.keys())
    counts = list(yearly_counts.values())
    axes[1, 1].bar(years, counts, alpha=0.7)
...
def regenerate_geographical_analysis(plot_data):
    """Regenerate geographical analysis figure from saved data"""
    # Get metadata for validation
    metadata = plot_data.get('metadata', {})
    figure_type = metadata.get('figure_type', 'geographical_analysis')
    title = metadata.get('title', 'Geographical Analysis - Music Preferences by Country')
    
    if figure_type != 'geographical_analysis':
        print(f"⚠️  Warning: Expected 'geographical_analysis' but got '{figure_type}'")
    
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle(title, fontsize=16, fontweight='bold')
    
    # 1. Top countries by song count
    country_counts = plot_data['country_counts']
    countries = list(country_counts.keys())[:15]
    counts = list(country_counts.values())[:15]
    axes[0, 0].barh(range(len(countries)), counts)
    axes[0, 0].set_yticks(range(len(countries)))
    axes[0, 0].set_yticklabels(countries, fontsize=10)
    axes[0, 0].set_xlabel('Number of Songs')
    axes[0, 0].set_title('Top 15 Countries by Song Count')
    axes[0, 0].grid(True, alpha=0.3)
    
    # 2. Country distribution pie chart
    top_10_countries = dict(list(country_counts.items())[:10])
    other_count = sum(list(country_counts.values())[10:])
    pie_data = list(top_10_countries.values()) + [other_count]
    pie_labels = list(top_10_countries.keys()) + ['Others']
    ...
    songs_per_country = list(country_counts.values())
    axes[1, 0].hist(songs_per_country, bins=20, alpha=0.7, edgecolor='black')
    ...
    # 4. Top countries by unique artists (if available)
    if 'unique_artists_per_country' in plot_data:
        unique_artists = plot_data['unique_artists_per_country']
        countries = list(unique_artists.keys())[:15]
        counts = list(unique_artists.values())[:15]
        axes[1, 1].barh(range(len(countries)), counts)
        axes[1, 1].set_yticks(range(len(countries)))
        axes[1, 1].set_yticklabels(countries, fontsize=10)
        axes[1, 1].set_xlabel('Number of Unique Artists')
        axes[1, 1].set_title('Top 15 Countries by Unique Artists')
        axes[1, 1].grid(True, alpha=0.3)
    else:
        axes[1, 1].text(0.5, 0.5, 'No artist data available', ha='center', va='center', transform=axes[1, 1].transAxes)
        axes[1, 1].set_title('Artist Analysis Not Available')
    
    plt.tight_layout()
    return fig

def regenerate_audio_features_analysis(plot_data):
    """Regenerate audio features analysis figure from saved data"""
    # Get metadata for validation
    metadata = plot_data.get('metadata', {})
    figure_type = metadata.get('figure_type', 'audio_features_analysis')
    title = metadata.get('title', 'Audio Features Correlation Analysis')
    
    if figure_type != 'audio_features_analysis':
        print(f"⚠️  Warning: Expected 'audio_features_analysis' but got '{figure_type}'")
    
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle(title, fontsize=16, fontweight='bold')
    
    # 1. Correlation heatmap
    correlation_matrix = plot_data['correlation_matrix']
    features = list(correlation_matrix.keys())
    corr_values = [list(row.values()) for row in correlation_matrix.values()]
    
    im = axes[0, 0].imshow(corr_values, cmap='coolwarm', aspect='auto', vmin=-1, vmax=1)
    ...
    for i in range(len(features)):
        for j in range(len(features)):
            text = axes[0, 0].text(j, i, f'{corr_values[i][j]:.2f}',
                                 ha="center", va="center", color="black", fontsize=8)
    
    plt.colorbar(im, ax=axes[0, 0])
    
    # 2-4. Feature distributions and scatter plots
    if 'feature_distributions' in plot_data:
        feature_dists = plot_data['feature_distributions']
        for i, (feature, data) in enumerate(feature_dists.items()):
            if i < 4:
    ...
    if 'scatter_data' in plot_data:
        scatter_data = plot_data['scatter_data']
        axes[1, 1].scatter(scatter_data['x'], scatter_data['y'], 
                          c=scatter_data['c'], cmap='viridis', alpha=0.6)
        axes[1, 1].set_xlabel(scatter_data['xlabel'])
        axes[1, 1].set_ylabel(scatter_data['ylabel'])
        axes[1, 1].set_title(scatter_data['title'])
        plt.colorbar(axes[1, 1].collections[0], ax=axes[1, 1])
    else:
        axes[1, 1].text(0.5, 0.5, 'No scatter plot data available', 
                       ha='center', va='center', transform=axes[1, 1].transAxes)
        axes[1, 1].set_title('Scatter Plot Not Available')
    
    plt.tight_layout()
    return fig

def regenerate_all_figures():
    """Regenerate all figures from saved plot data"""
    print("🔄 Regenerating all figures from saved data...")
    
    try:
        # Regenerate basic data overview
        print("  📊 Regenerating basic data overview...")
        plot_data = load_plot_data('basic_data_overview', 'data')
        fig = regenerate_basic_data_overview(plot_data)
        save_figure(fig, 'basic_data_overview_regenerated', 'figures/basic_analysis')
        print("  ✅ Basic data overview regenerated")
        
        # Regenerate temporal analysis
        print("  📈 Regenerating temporal analysis...")
        plot_data = load_plot_data('temporal_analysis', 'data')
        fig = regenerate_temporal_analysis(plot_data)
        save_figure(fig, 'temporal_analysis_regenerated', 'figures/temporal')
        print("  ✅ Temporal analysis regenerated")
        
        # Regenerate geographical analysis
        print("  🌍 Regenerating geographical analysis...")
        plot_data = load_plot_data('geographical_analysis', 'data')
        fig = regenerate_geographical_analysis(plot_data)
        save_figure(fig, 'geographical_analysis_regenerated', 'figures/geographical')
        print("  ✅ Geographical analysis regenerated")
        
        # Regenerate audio features analysis
        print("  🎵 Regenerating audio features analysis...")
        plot_data = load_plot_data('audio_features_analysis', 'data')
        fig = regenerate_audio_features_analysis(plot_data)
        save_figure(fig, 'audio_features_analysis_regenerated', 'figures/audio_features')
        print("  ✅ Audio features analysis regenerated")
        
        print("🎉 All figures regenerated successfully!")
        
    except FileNotFoundError as e:
        print(f"❌ Error: Could not find saved plot data. Please run the full EDA analysis first.")
        print(f"Missing file: {e}")
    except Exception as e:
        print(f"❌ Error regenerating figures: {e}")

def regenerate_specific_figure(figure_name):
    """Regenerate a specific figure from saved plot data"""
    print(f"🔄 Regenerating {figure_name} figure...")
    
    try:
        if figure_name == 'basic_data_overview':
            plot_data = load_plot_data('basic_data_overview', 'data')
            fig = regenerate_basic_data_overview(plot_data)
            save_figure(fig, f'{figure_name}_regenerated', 'figures/basic_analysis')
        elif figure_name == 'temporal_analysis':
            plot_data = load_plot_data('temporal_analysis', 'data')
            fig = regenerate_temporal_analysis(plot_data)
            save_figure(fig, f'{figure_name}_regenerated', 'figures/temporal')
        elif figure_name == 'geographical_analysis':
            plot_data = load_plot_data('geographical_analysis', 'data')
            fig = regenerate_geographical_analysis(plot_data)
            save_figure(fig, f'{figure_name}_regenerated', 'figures/geographical')
        elif figure_name == 'audio_features_analysis':
            plot_data = load_plot_data('audio_features_analysis', 'data')
            fig = regenerate_audio_features_analysis(plot_data)
            save_figure(fig, f'{figure_name}_regenerated', 'figures/audio_features')
        else:
            print(f"❌ Unknown figure name: {figure_name}")
            print("Available figures: basic_data_overview, temporal_analysis, geographical_analysis, audio_features_analysis")
            return
        
        print(f"✅ {figure_name} figure regenerated successfully!")
        
    except FileNotFoundError as e:
        print(f"❌ Error: Could not find saved plot data for {figure_name}")
        print(f"Missing file: {e}")
    except Exception as e:
        print(f"❌ Error regenerating {figure_name}: {e}")
...
# We'll check status after creating the output directory
...
if df is not None:
    ...
    # Check if we should use existing results or create new ones
    state = update_manager.load_analysis_state()
    existing_output_dir = state.get('output_directory')
    
    # Check if existing results are still valid
    if existing_output_dir and os.path.exists(existing_output_dir):
        print(f"\n📁 Found existing results: {existing_output_dir}")
        # Use existing output directory instead of creating new one
        output_dirs['base'] = existing_output_dir
        # Update the output_dirs dictionary to point to existing directory
        for key in output_dirs:
            if key != 'base':
                output_dirs[key] = os.path.join(existing_output_dir, key)
        
        # Check status with existing directory
        status = update_manager.get_analysis_status(path, output_dirs['base'])
        update_manager.print_status_report(status)
        
        # Determine what needs to be run
        if status['dataset_changed']:
            print(f"\n🔄 Dataset has changed: {status['change_reason']}")
            print("📊 Running full analysis due to dataset changes...")
            analyses_to_run = update_manager.analysis_components
        else:
            print(f"\n✅ Dataset unchanged. Running only missing analyses...")
            analyses_to_run = status['missing_analyses']
    else:
        print(f"\n📁 No existing results found, creating new analysis...")
        # Check status with new directory
        status = update_manager.get_analysis_status(path, output_dirs['base'])
        update_manager.print_status_report(status)
    ...
    if not analyses_to_run:
        print("\n🎉 All analyses are up to date! No processing needed.")
        print("💡 Use regenerate_all_figures() to recreate figures from saved data.")
    else:
        print(f"\n⏳ Running {len(analyses_to_run)} analyses: {', '.join(analyses_to_run)}")
    
    def run_analysis_with_tracking(analysis_name, analysis_function, *args, **kwargs):
        """Run analysis with incremental update tracking"""
        if analysis_name in analyses_to_run:
            ...
            try:
                ...
                update_manager.update_analysis_state(path, output_dirs['base'], analysis_name, analysis_time)
                print(f"✅ {analysis_name} completed in {analysis_time:.1f} seconds")
    ...
    if 'basic_data_overview' in analyses_to_run:
        print("\n" + "="*60)
    ...
    def basic_data_overview(df):
        ...
        # Prepare plot data for regeneration
        plot_data = {
            'metadata': {
                'figure_type': 'basic_data_overview',
                'title': 'Basic Data Overview - Spotify Dataset',
                'subplot_layout': '2x3',
                'total_subplots': 6,
                'subplot_info': {
                    '0,0': {'type': 'barh', 'title': 'Missing Values by Column', 'xlabel': 'Missing Percentage (%)'},
                    '0,1': {'type': 'pie', 'title': 'Data Types Distribution'},
                    '0,2': {'type': 'line', 'title': 'Dataset Size Over Time', 'xlabel': 'Date', 'ylabel': 'Number of Records'},
                    '1,0': {'type': 'hist', 'title': 'Distribution of Numeric Column 1', 'xlabel': 'Value', 'ylabel': 'Frequency'},
                    '1,1': {'type': 'hist', 'title': 'Distribution of Numeric Column 2', 'xlabel': 'Value', 'ylabel': 'Frequency'},
                    '1,2': {'type': 'barh', 'title': 'Top 10 Categories', 'xlabel': 'Count'}
                }
            },
            'missing_data': missing_data.to_dict(),
            'missing_percent': missing_percent.tolist(),
        ...
        # Add daily counts if available
        if date_cols:
            plot_data['daily_counts'] = daily_counts.to_dict()
        
        # Add numeric distributions
        if len(numeric_cols) > 0:
            cols_to_plot = numeric_cols[:4] if len(numeric_cols) >= 4 else numeric_cols
            for col in cols_to_plot:
                plot_data['numeric_distributions'][col] = {
        ...
        if len(categorical_cols) > 0:
            top_categories = df[categorical_cols[0]].value_counts().head(10)
            plot_data['categorical_data'] = top_categories.to_dict()
        
        # Save plot data for regeneration
        save_plot_data(plot_data, 'basic_data_overview', 'data')
    ...
    if 'basic_data_overview' in analyses_to_run:
        ...
        # Update analysis state
        analysis_time = time.time() - start_time
        update_manager.update_analysis_state(path, output_dirs['base'], 'basic_data_overview', analysis_time)
        print(f"✅ Basic data overview completed in {analysis_time:.1f} seconds")
    else:
        print("⏭️  Skipping basic data overview (already completed)")
        # Load existing results if needed for other analyses
        missing_df, numeric_cols, categorical_cols = None, None, None
    
    # 2. TEMPORAL ANALYSIS OF SONG POPULARITY TRENDS
    if 'temporal_analysis' in analyses_to_run:
        print("\n" + "="*60)
    ...
    def temporal_analysis(df):
        ...
        plot_data = {
            'metadata': {
                'figure_type': 'temporal_analysis',
                'title': 'Temporal Analysis - Song Popularity Trends',
                'subplot_layout': '2x2',
                'total_subplots': 4,
                'subplot_info': {
                    '0,0': {'type': 'line', 'title': 'Daily Song Counts Over Time', 'xlabel': 'Date', 'ylabel': 'Number of Songs'},
                    '0,1': {'type': 'bar', 'title': 'Weekly Song Distribution', 'xlabel': 'Week Number', 'ylabel': 'Number of Songs'},
                    '1,0': {'type': 'bar', 'title': 'Monthly Song Distribution', 'xlabel': 'Month', 'ylabel': 'Number of Songs'},
                    '1,1': {'type': 'bar', 'title': 'Yearly Song Distribution', 'xlabel': 'Year', 'ylabel': 'Number of Songs'}
                }
            },
        ...
        # Save plot data for regeneration
        save_plot_data(plot_data, 'temporal_analysis', 'data')
    ...
    if 'temporal_analysis' in analyses_to_run:
        ...
        # Update analysis state
        analysis_time = time.time() - start_time
        update_manager.update_analysis_state(path, output_dirs['base'], 'temporal_analysis', analysis_time)
        print(f"✅ Temporal analysis completed in {analysis_time:.1f} seconds")
    else:
        print("⏭️  Skipping temporal analysis (already completed)")
        temporal_results = None
    
    # 3. GEOGRAPHICAL/COUNTRY-WISE MUSIC PREFERENCES ANALYSIS
    if 'geographical_analysis' in analyses_to_run:
        print("\n" + "="*60)
    ...
    def geographical_analysis(df):
        ...
        plot_data = {
            'metadata': {
                'figure_type': 'geographical_analysis',
                'title': 'Geographical Analysis - Music Preferences by Country',
                'subplot_layout': '2x2',
                'total_subplots': 4,
                'subplot_info': {
                    '0,0': {'type': 'barh', 'title': 'Top 15 Countries by Song Count', 'xlabel': 'Number of Songs'},
                    '0,1': {'type': 'pie', 'title': 'Country Distribution (Top 10 + Others)'},
                    '1,0': {'type': 'hist', 'title': 'Distribution of Songs per Country', 'xlabel': 'Number of Songs', 'ylabel': 'Number of Countries'},
                    '1,1': {'type': 'barh', 'title': 'Top 15 Countries by Unique Artists', 'xlabel': 'Number of Unique Artists'}
                }
            },
            'country_counts': country_counts.to_dict()
        }
        
        # Add unique artists per country if available
        if artist_cols:
            unique_artists_per_country = df.groupby(country_col)[artist_col].nunique().sort_values(ascending=False).head(15)
            plot_data['unique_artists_per_country'] = unique_artists_per_country.to_dict()
        
        # Save plot data for regeneration
        save_plot_data(plot_data, 'geographical_analysis', 'data')
    ...
    geographical_results = run_analysis_with_tracking('geographical_analysis', geographical_analysis, df_clean)
    
    # 4. AUDIO FEATURES CORRELATION ANALYSIS
    if 'audio_features_analysis' in analyses_to_run:
        print("\n" + "="*60)
    ...
    def audio_features_analysis(df):
        ...
        plot_data = {
            'metadata': {
                'figure_type': 'audio_features_analysis',
                'title': 'Audio Features Correlation Analysis',
                'subplot_layout': '2x2',
                'total_subplots': 4,
                'subplot_info': {
                    '0,0': {'type': 'imshow', 'title': 'Audio Features Correlation Heatmap'},
                    '0,1': {'type': 'hist', 'title': 'Distribution of Audio Feature 1', 'xlabel': 'Value', 'ylabel': 'Frequency'},
                    '1,0': {'type': 'hist', 'title': 'Distribution of Audio Feature 2', 'xlabel': 'Value', 'ylabel': 'Frequency'},
                    '1,1': {'type': 'scatter', 'title': 'Feature Correlation Scatter Plot', 'xlabel': 'Feature 1', 'ylabel': 'Feature 2'}
                }
            },
        ...
        if len(available_features) >= 4:
            for feature in available_features[:4]:
                plot_data['feature_distributions'][feature] = df[feature].dropna().tolist()
        
        # Add scatter plot data if available
        if popularity_cols and len(available_features) >= 2:
            sample_df = df.sample(min(1000, len(df)))
            plot_data['scatter_data'] = {
                'x': sample_df[available_features[0]].tolist(),
                'y': sample_df[available_features[1]].tolist(),
                'c': sample_df[popularity_col].tolist(),
                'xlabel': available_features[0],
                'ylabel': available_features[1],
                'title': f'{available_features[0]} vs {available_features[1]} (colored by {popularity_col})'
            }
        
        # Save plot data for regeneration
        save_plot_data(plot_data, 'audio_features_analysis', 'data')
    ...
    audio_results = run_analysis_with_tracking('audio_features_analysis', audio_features_analysis, df_clean)
    
    # 5. FEATURE SELECTION METHODS
    if 'feature_selection_analysis' in analyses_to_run:
        print("\n" + "="*60)
    ...
    def feature_selection_analysis(df):
        ...
        max_samples = 20000  # Much smaller sample for speed
        if len(df) > max_samples:
            print(f"📊 Dataset large ({len(df):,} rows), sampling {max_samples:,} rows for feature selection...")
        ...
        try:
            ...
            y = df_sample[target_col].fillna(df_sample[target_col].median())
            ...
            if len(feature_cols) < 2:
                print("Not enough features after removing constant columns")
                return
        ...
        if len(feature_cols) > 5:
            if GPU_AVAILABLE and CUML_AVAILABLE and CUDF_AVAILABLE:
                ...
                # Optimized CPU version for large datasets with memory management
                estimator = RandomForestRegressor(
                    n_estimators=25,  # Further reduced for speed
                    random_state=42,
                    max_depth=8,  # Further reduced for speed
                    n_jobs=-1,  # Use all CPU cores
                    max_samples=0.6  # Use 60% of samples for speed
                )
                selector_rfe = RFE(estimator, n_features_to_select=min(10, len(feature_cols)))
                selector_rfe.fit(X, y)
                selected_features_rfe = [feature_cols[i] for i in range(len(feature_cols)) if selector_rfe.support_[i]]
        ...
        for i, col in enumerate(feature_cols):
            if i % 10 == 0:  # Progress indicator every 10 features
                print(f"   📊 Processing feature {i+1}/{total_features}: {col}")
            try:
                # Ensure we have proper numpy arrays
                x_values = np.array(X[col], dtype=float)
                ...
                if len(x_values) > 1 and len(y_values) > 1:
                    ...
                    if np.std(x_values) > 0 and np.std(y_values) > 0:
                        ...
                        print(f"⚠️  Skipping {col}: constant values detected")
                else:
                    print(f"⚠️  Skipping {col}: insufficient data points")
            except Exception as e:
                print(f"⚠️  Error computing correlation for {col}: {e}")
                continue
    ...
    feature_selection_results = run_analysis_with_tracking('feature_selection_analysis', feature_selection_analysis, df_clean)
    
    # 6. PREDICTIVE ANALYSIS MODELS
    if 'predictive_analysis' in analyses_to_run:
        print("\n" + "="*60)
    ...
    def predictive_analysis(df):
        ...
        max_samples = 20000  # Much smaller sample for speed
        if len(df) > max_samples:
            print(f"📊 Dataset large ({len(df):,} rows), sampling {max_samples:,} rows for predictive analysis...")
            df_sample = df.sample(n=max_samples, random_state=42)
        else:
            df_sample = df
        ...
        if GPU_AVAILABLE and CUML_AVAILABLE and CUDF_AVAILABLE:
            ...
            models = {
                ...
                'Random Forest': RandomForestRegressor(n_estimators=20, random_state=42, max_depth=6, n_jobs=-1, max_samples=0.5)
            }
        ...
        for i, (name, model) in enumerate(models.items()):
            ...
            if name == 'Linear Regression':
                ...
                if GPU_AVAILABLE and CUML_AVAILABLE and CUDF_AVAILABLE:
                    try:
                        ...
                        # Fallback to CPU
                        cpu_model = RandomForestRegressor(n_estimators=20, random_state=42, max_depth=6, n_jobs=-1, max_samples=0.5)
            ...
            try:
                # Use minimal folds for Random Forest to speed up
                cv_folds = 2 if name == 'Random Forest' else 3
                if name == 'Linear Regression':
                ...
                results[name]['CV_R2_Mean'] = 0
                results[name]['CV_R2_Std'] = 0
        ...
        # Save the figure
        print("💾 Saving predictive analysis figure...")
    ...
    predictive_results = run_analysis_with_tracking('predictive_analysis', predictive_analysis, df_clean)
    
    # 7. GENRE AND ARTIST POPULARITY PATTERNS
    if 'genre_artist_analysis' in analyses_to_run:
        print("\n" + "="*60)
    ...
    genre_artist_results = run_analysis_with_tracking('genre_artist_analysis', genre_artist_analysis, df_clean)
    
    # 8. EXPLICITNESS VS POPULARITY RELATIONSHIPS
    if 'explicitness_analysis' in analyses_to_run:
        print("\n" + "="*60)
    ...
    explicitness_results = run_analysis_with_tracking('explicitness_analysis', explicitness_analysis, df_clean)
    
    # 9. COMPREHENSIVE VISUALIZATION DASHBOARD
    if 'comprehensive_dashboard' in analyses_to_run:
        print("\n" + "="*60)
    ...
    run_analysis_with_tracking('comprehensive_dashboard', create_comprehensive_dashboard, df_clean)
    ...
    def generate_final_summary():
        ...
        print(f"\n🔄 FIGURE REGENERATION:")
        print(f"   • All plot data has been saved for regeneration")
        print(f"   • Use regenerate_all_figures() to recreate all figures")
        print(f"   • Use regenerate_specific_figure('figure_name') for individual figures")
        print(f"   • Available figures: basic_data_overview, temporal_analysis, geographical_analysis, audio_features_analysis")
...
"""
    ...
    # Final incremental update status
    print("\n" + "="*60)
    print("📊 FINAL INCREMENTAL UPDATE STATUS")
    print("="*60)
    final_status = update_manager.get_analysis_status(path, output_dirs['base'])
    update_manager.print_status_report(final_status)
    
    if final_status['completion_percentage'] == 100:
        print("\n🎉 All analyses completed successfully!")
        print("💡 Next time you run this script, it will only process new data or missing analyses.")
    else:
        print(f"\n⚠️  {100 - final_status['completion_percentage']:.1f}% of analyses still pending.")
        print("💡 Run the script again to complete remaining analyses.")
...
def check_analysis_status():
    """Check current analysis status without running analysis"""
    print("🔍 Checking analysis status...")
    # This would need the dataset path, but we can show a general status
    try:
        state = update_manager.load_analysis_state()
        if state['last_update']:
            print(f"📅 Last update: {state['last_update']}")
            print(f"📊 Completed analyses: {len(state['completed_analyses'])}/{len(update_manager.analysis_components)}")
            print(f"✅ Completed: {', '.join(state['completed_analyses'])}")
            missing = [comp for comp in update_manager.analysis_components if comp not in state['completed_analyses']]
            if missing:
...
def force_full_analysis():
    """Force a full analysis run (ignore incremental updates)"""
    print("🔄 Forcing full analysis...")
    # Clear the analysis state to force full run
    state = update_manager.load_analysis_state()
    state['completed_analyses'] = []
    state['dataset_hash'] = None
    update_manager.save_analysis_state(state)
    print("✅ Analysis state cleared. Next run will perform full analysis.")

def clean_old_results():
    """Clean up old analysis results"""
    print("🧹 Cleaning old analysis results...")
    try:
        state = update_manager.load_analysis_state()
        if state.get('output_directory') and os.path.exists(state['output_directory']):
            import shutil
            shutil.rmtree(state['output_directory'])
            print(f"✅ Removed old results: {state['output_directory']}")
...
def show_incremental_status():
    """Show detailed incremental update status"""
    print("🔍 Detailed Incremental Update Status")
    print("="*50)
    ...
    if state.get('last_update'):
        print(f"📅 Last Update: {state['last_update']}")
        print(f"📁 Output Directory: {state.get('output_directory', 'None')}")
        print(f"🔑 Dataset Hash: {state.get('dataset_hash', 'None')[:16]}...")
        
        print(f"\n📊 Completed Analyses ({len(state['completed_analyses'])}/{len(update_manager.analysis_components)}):")
        for analysis in state['completed_analyses']:
            if analysis in state.get('analysis_times', {}):
                duration = state['analysis_times'][analysis]['duration_seconds']
                print(f"   ✅ {analysis} ({duration:.1f}s)")
            else:
                print(f"   ✅ {analysis}")
        
        missing = [comp for comp in update_manager.analysis_components if comp not in state['completed_analyses']]
        if missing:
            print(f"\n⏳ Missing Analyses ({len(missing)}):")
            for analysis in missing:
                print(f"   ❌ {analysis}")
        
        # Check if output directory exists
        if state.get('output_directory') and os.path.exists(state['output_directory']):
            print(f"\n📁 Output Directory Status: ✅ EXISTS")
            print(f"   Path: {state['output_directory']}")
        else:
            print(f"\n📁 Output Directory Status: ❌ MISSING")
            print(f"   Path: {state.get('output_directory', 'None')}")
    else:
        print("📊 No previous analysis found")
        print("💡 This will be treated as a first run")
    
    print("="*50)

# Example usage for incremental updates and figure regeneration (uncomment to use):
"""
# Check analysis status:
# check_analysis_status()

# Show detailed incremental status:
...
# To regenerate all figures from saved data:
# regenerate_all_figures()

# To regenerate a specific figure:
# regenerate_specific_figure('basic_data_overview')
# regenerate_specific_figure('temporal_analysis')
# regenerate_specific_figure('geographical_analysis')
# regenerate_specific_figure('audio_features_analysis')

# To inspect graph type information:
# get_graph_type_info('basic_data_overview', 'data')
# list_available_graph_types('data')

# To load and inspect saved plot data:
# plot_data = load_plot_data('basic_data_overview', 'data')
# print(plot_data.keys())
# print(plot_data['metadata'])  # View graph type metadata
"""